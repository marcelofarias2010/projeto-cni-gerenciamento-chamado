import { Component } from '@angular/core';
import { CommonModule } from '@angular/common'; // Necessário para o @if
import { RouterOutlet, RouterModule } from '@angular/router';
import { ButtonModule } from 'primeng/button'; // Necessário para o botão do ícone

@Component({
  selector: 'app-root',
  standalone: true,
  // Adicione ButtonModule e CommonModule e RouterModule aos imports
  imports: [CommonModule, RouterOutlet, RouterModule, ButtonModule],
  template: `
    <div class="flex flex-column min-h-screen relative">
      
      <header class="cni-header p-3 flex align-items-center justify-content-between relative z-2">
        <div class="flex align-items-center gap-3">
          <h1 class="m-0 text-3xl font-bold italic titulo">CNI</h1>
          <span class="opacity-70 text-sm hidden md:block border-left-1 pl-3">Confederação Nacional da Indústria</span>
        </div>
        
        <p-button 
            icon="pi pi-bars" 
            styleClass="p-button-rounded p-button-text text-white md:hidden text-2xl"
            (click)="toggleMobileMenu()">
        </p-button>

        <nav class="hidden md:flex gap-4 text-sm font-semibold">
          <a class="cursor-pointer hover:text-primary-300 transition-colors">Institucional</a>
          <a class="cursor-pointer hover:text-primary-300 transition-colors">Atuação</a>
          <a class="cursor-pointer hover:text-primary-300 transition-colors">Notícias</a>
        </nav>
      </header>

      @if (isMobileMenuOpen) {
        <div class="cni-mobile-menu md:hidden fadein animation-duration-200 shadow-4">
            <nav class="flex flex-column p-4 gap-4 font-semibold text-lg">
              <a class="cursor-pointer text-white hover:text-primary-300 transition-colors" (click)="toggleMobileMenu()">Institucional</a>
              <a class="cursor-pointer text-white hover:text-primary-300 transition-colors" (click)="toggleMobileMenu()">Atuação</a>
              <a class="cursor-pointer text-white hover:text-primary-300 transition-colors" (click)="toggleMobileMenu()">Notícias</a>
            </nav>
        </div>
      }

      <main class="flex-grow-1 p-3 md:p-6 fadein animation-duration-500 z-1">
        <div class="max-w-70rem mx-auto">
          <router-outlet></router-outlet>
        </div>
      </main>

      <footer class="cni-footer text-center md:text-left z-1">
        <div class="max-w-70rem mx-auto flex flex-column md:flex-row justify-content-between align-items-center gap-3">
          <div>
            <p class="m-0 font-bold text-xl italic">CNI</p>
            <small class="opacity-80">Sistema de Gestão de Chamados © 2025</small>
          </div>
          <div class="flex gap-3">
             <i class="pi pi-facebook cursor-pointer text-xl"></i>
             <i class="pi pi-linkedin cursor-pointer text-xl"></i>
             <i class="pi pi-instagram cursor-pointer text-xl"></i>
          </div>
        </div>
      </footer>

    </div>
  `
})
export class AppComponent {
  // Estado para controlar se o menu mobile está visível
  isMobileMenuOpen = false;

  // Função para alternar o estado (abrir/fechar)
  toggleMobileMenu() {
    this.isMobileMenuOpen = !this.isMobileMenuOpen;
  }
}